...
export const BlogPostClient: React.FC<BlogPostClientProps> = ({ post }) => {
  ...
  return (
    <>
      <article>
        <header className="relative min-h-[460px] md:min-h-[520px] w-full flex items-center justify-center bg-[#070F1A] overflow-hidden pt-36 sm:pt-40 md:pt-48 pb-14 sm:pb-16 md:pb-20">
          <Image
            src={post.image}
            alt={post.title}
            fill
      ...
      {related.length > 0 ? (
        <aside aria-label="More Articles" className="py-12 sm:py-16 bg-[#070F1A] text-white relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 sm:gap-6">
              {related.map((item) => (
                <Link
                  key={item.slug}
                  href={`/blog/${item.slug}`}
                  className="group rounded-sm border border-[#C5A059]/25 bg-[#0C1827] overflow-hidden hover:border-[#E8A317]/50 transition-colors block"
                >
                  <article>
